...
from schemas.page_operation import CreatePageOperationRequest, QueryPageOperationRequest, QueryPageOperationsResponse, UpdatePageOperationRequest
...
@router.get('')
async def query(*, db:Session=Depends(get_db), request: Annotated[QueryPageOperationRequest, Depends()]) -> QueryPageOperationsResponse: 
...
async def create(*, db:Session=Depends(get_db), request: CreatePageOperationRequest): 
    page_operation_service.create(db=db, request=request)
